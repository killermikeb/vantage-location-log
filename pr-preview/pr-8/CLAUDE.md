...
- **Data format**: the single source of truth is currently a plain-text blob stored in `localStorage` under `vantage_location_text` (there is no backup/sync file yet — see below), using this line syntax, one location per line, grouped under `// <ordinal day> <Mon> <year>` date-header comments (e.g. `// 15th Feb 2026`):
  ```
  <id> N:<target|---|***> E:<target|---|***> S:<target|---|***> W:<target|---|***> T:<type> [R:<region>] [A:<label>-><target>,...] [B:<label>-><description>,...]
  ```
  ...
  - `R:` is an optional single-letter region, picked from a fixed `<select>` in the Add form (not autocomplete, since the set is closed): `A`-`P`, or `S` (Sky) / `U` (Underground). The Graph tab's "Group by region" toggle (`g-regionToggle`) arranges the map by region instead of leaving it to the force layout — `A`-`P` as a 4x4 grid (`A B C D` / `E F G H` / `I J K L` / `M N O P`, row by row, see `REGION_GRID_LETTERS` in [app.js](app.js)), `S` above it, `U` below it — via `regionGridCell()`/`applyRegionForces()` in `renderGraph()`. The in-game regions actually wrap like a torus (east off `D` reaches `A`, south off `M` reaches `A`, etc.), which this grid doesn't render as a wraparound — that's a known simplification that will need revisiting once enough of the map is logged to make it matter.
  - This format is deliberately compatible with `vantage-graph.html`'s `const input = \`...\`;` template so data can be copy-pasted between the two files directly (`vantage-graph.html` parses and ignores `R:` like it does `T:`/`B:`, but doesn't visualise regions — that UI only exists in `app.js`'s Graph tab).
